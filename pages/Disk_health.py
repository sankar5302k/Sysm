...
TEMP_THRESHOLD = 85  # CPU temperature threshold in Celsius
...
def get_cpu_temperature():
    ...
    try:
        ...
        temp_celsius = (temp_info.CurrentTemperature / 10.0) - 273.15  # Convert from Kelvin
        return temp_celsius
...
def check_disk_health():
    """Check disk health using SMART status via WMI and simulate specific issues."""
    try:
        w = wmi.WMI()
        disks = w.Win32_DiskDrive()
        disk_status = {}
        for disk in disks:
            status = disk.Status  # General status
            specific_issues = []
